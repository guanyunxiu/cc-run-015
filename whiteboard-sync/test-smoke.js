...
async function main() {
  ...
  // 模拟 D 本地 history 中有 moveD；undo() 返回信封组，需 flat 后逐条本地物化并广播
  const invGroup = um.undo(new Map([[moveD.id, moveD]])).flat();
  for (const inv of invGroup) D._ingest(inv);
  send(D.ws, { type: 'ops', envelopes: invGroup });
  await sleep(200);
  ...
  console.log('\n[5b] 像素擦并发撤销：A、B 擦同一分块的不同单元，A 撤销只恢复自己的格子');
  const strokePts2 = [];
  for (let i = 0; i <= 40; i++) strokePts2.push({ x: i * 6, y: 400 });
  A.issue({ kind: 'create', objects: [{ oid: 'eraseUndo', type: 'stroke', fields: { stroke: { width: 8, cellSize: 8, points: strokePts2 } } }] });
  await sleep(200);
  // A 擦 tile(0,0) 的单元 (1,1)/(2,2)
  const erA2 = A.issue({ kind: 'erase', chunks: [{ oid: 'eraseUndo', tx: 0, ty: 0, cells: [[1, 1], [2, 2]] }] });
  await sleep(200);
  // B 随后擦同一分块的不同单元 (9,9)（因果在 A 之后）
  B.issue({ kind: 'erase', chunks: [{ oid: 'eraseUndo', tx: 0, ty: 0, cells: [[9, 9]] }] });
  await sleep(200);
  assert(A.doc.erasedCells('eraseUndo').size === 3, 'pre-undo: 3 cells erased on A');
  // A 撤销自己的擦除并广播
  const umE = new WB.UndoManager(A.clock);
  umE.record([erA2], 'erase');
  const invE = umE.undo(new Map([[erA2.id, erA2]])).flat();
  for (const e of invE) A._ingest(e);
  send(A.ws, { type: 'ops', envelopes: invE });
  await sleep(250);
  const want = ['0:0,9,9']; // A 擦的 (1,1)/(2,2) 恢复；B 擦的 (9,9) 保留
  const finalA = [...A.doc.erasedCells('eraseUndo')].sort();
  const finalB = [...B.doc.erasedCells('eraseUndo')].sort();
  const finalF = [...F.doc.erasedCells('eraseUndo')].sort();
  assert(JSON.stringify(finalA) === JSON.stringify(want), `A's undo restores only its own cells locally (got ${finalA})`);
  assert(JSON.stringify(finalB) === JSON.stringify(want), `B converges: A's cells restored, B's (9,9) kept (got ${finalB})`);
  assert(JSON.stringify(finalF) === JSON.stringify(want), `passive replica F converges too (got ${finalF})`);
  ...
  if (serverProc) serverProc.kill(); // 回收本测试拉起的服务端，不留孤儿进程
...
ensureServer().then(main).catch((err) => {
