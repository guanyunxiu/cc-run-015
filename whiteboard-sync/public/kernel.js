 ...
 *  - 保护粒度：普通字段按字段级、create 按对象级、像素擦除按擦除单元级
 *    （同一分块内他人只擦了别的单元时，自己的撤销对未冲突单元仍然生效）。
 *
...
(function (root, factory) {
  ...
  class Doc {
    ...
    /**
     * 判断一条逆操作（undo / redo）是否被架空（字段级，用于 set/layer/delete 等普通寄存器；
     * 像素擦除的逆操作粒度更细——按擦除单元判定，见 _inverseActiveCells）。
     * @param {object} rec 逆写入记录
     * @param {object} o   对象
     * @param {boolean} objectWide create/delete 类逆操作按对象级保护
     */
    _inverseVoid(rec, o, wide, fieldKey) {
      const inv = rec.inv;
      if (!inv) return false;
      const originL = inv.originLamport;

      // wide（create 的撤销）：对象上任意寄存器（含像素擦除块）被他人写过即空转；
      // 窄（set/layer 的撤销）：只看逆写入所在的那一个寄存器。
      const fields = wide
      ...
      for (const f of fields) {
        ...
        for (const r of arr) {
          if (r.id === rec.id || r.id === inv.originId) continue;
    ...
    /**
     * 像素擦逆操作的“单元级”架空检测（选择性撤销粒度 = 擦除单元，而非整个分块寄存器）。
     * 同一分块内，他人只擦了别的单元时，本逆操作对未冲突单元仍然生效；
     * 只有被他人（在原操作之后、含并发更大 lamport 的普通写入）覆盖的单元才空转。
     * @param {object} rec 逆写入记录（erase/unerase，inv 非空）
     * @param {object} o   对象
     * @param {string} key 所在分块寄存器（'erase:tx:ty'）
     * @returns {Array} 本逆操作仍然生效的单元子集
     */
    _inverseActiveCells(rec, o, key) {
      const inv = rec.inv;
      const originL = inv.originLamport;
      const arr = o.regs.get(key) || [];
      let guarded = null; // 他人保护性写入覆盖的单元 "cx,cy"
      for (const r of arr) {
        if (r.id === rec.id || r.id === inv.originId) continue;
        if (r.clientId === rec.clientId) continue; // 只防“别人”
        if (r.inv) continue;                        // 别人的撤销/重做不构成保护性写入
        if (r.lamport < originL) continue;          // 只防原操作之后（含并发更大 lamport）
        if (!guarded) guarded = new Set();
        for (const c of r.value) guarded.add(c[0] + ',' + c[1]);
      }
      if (!guarded) return rec.value;
      return rec.value.filter((c) => !guarded.has(c[0] + ',' + c[1]));
    }
    ...
    erasedCells(oid) {
      ...
      const set = new Set();
      if (!o) return set;
      // 每个擦除单元独立 LWW 折叠；逆操作先按单元做架空过滤——
      // 同块内他人擦的其它单元不影响本次恢复，只跳过真正冲突的单元
      const perCell = new Map();
      for (const key of o.regs.keys()) {
        if (!key.startsWith('erase:')) continue;
        for (const r of o.regs.get(key)) {
          const cells = r.inv ? this._inverseActiveCells(r, o, key) : r.value;
     ...
     * 每个寄存器只保留当前获胜记录；擦除寄存器只保留每格 LWW 获胜者。
     * 获胜记录已携带 (lamport, clientId)，足以继续做 LWW 仲裁与撤销架空保护。
     */
    snapshot(knownVC) {
      const objects = [];
      for (const [oid, o] of this.objs) {
        ...
        for (const [key, arr] of o.regs) {
          if (key.startsWith('erase:')) {
            ...
            // 每个单元折叠到当前获胜（erase vs unerase）；
            // 逆操作先按单元架空过滤，与 erasedCells 同一判定——快照不固化被架空的撤销
            const perCell = new Map();
            for (const r of arr) {
              const cells = r.inv ? this._inverseActiveCells(r, o, key) : r.value;
              for (const c of cells) {
                const ck = c[0] + ',' + c[1];
                const prev = perCell.get(ck);
                if (!prev || tsNewer(r.lamport, r.clientId, prev.rec.lamport, prev.rec.clientId)) {
                  perCell.set(ck, { cell: c, rec: r });
                }
              }
            }
            if (perCell.size) {
              const cells = [];
              for (const v of perCell.values()) {
                cells.push({ cell: v.cell, id: v.rec.id, lamport: v.rec.lamport,
                  clientId: v.rec.clientId, unerase: !!v.rec.unerase, inv: v.rec.inv || null });
              }
              erases[tileKey] = cells;
            }
            continue;
          }
